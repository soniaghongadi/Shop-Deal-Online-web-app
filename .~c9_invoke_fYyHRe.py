...
def upload(request):
    if request.method == "POST":
        ...
        print(S3Utils.upload_file(BUCKET, path, f.filename))
...
def addproduct(): 
    if request.method == 'POST':
        ...
        if not request.form['product_type'] or not request.form['Product_price'] or not request.form['Prod_spec'] or not request.form['Seller_name'] or not request.form['Seller_Email'] or not request.form['Contact_num']:
            ...
            if web_urlupload(request)
            web_url = upload(request)
             # Add to the database
            table_product.put_item(Item= {'id': product_id,'Department':Department, 'product_type': product_type, 'Product_price': Product_price, 
                                    'Prod_spec': Prod_spec, 'Seller_name': Seller_name , 'Seller_Email':Seller_Email, 'Contact_num': Contact_num, 'image_link':web_url
                                })
            if SMS_ACTIVATE:
...
def update_prod(product_id):
    ...
    if request.method == 'POST':
        ...
        db_result = table_product.update_item(
            ...
            UpdateExpression="set Product_name=:pName, Product_price=:pPrice, Prod_spec=:pSpec, Seller_name=:pSname, Seller_Email=:pSemail, Contact_num=:pCnum",
